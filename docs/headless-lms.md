...
If the type is to be used by the frontend, `ts_rs::TS` should also be derived for the type. In addition, the type should be added to the export macro in `ts_binding_generator.rs`. Running the test generated by the macro automatically generates up to date bindings to the backend types in shared-module.
