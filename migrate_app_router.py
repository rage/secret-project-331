...
# ---------------- Docs Context ----------------
# Paste the official migration docs here to improve model accuracy. Leave empty if not needed.
NEXT_DOCS = """
...
def azure_chat_completion(messages: List[Dict[str, str]], temperature: float = 0.2, max_retries: int = 3) -> str:
...
def user_prompt_for_file(rel_path: str, original: str) -> str:
    return f"""We are migrating from Pages Router to the App Router.

Repository path: {rel_path}

IMPORTANT: The file path above shows the route structure. Dynamic route segments are indicated by brackets:
...
- ONLY use useParams<{{ id: string }}>() if BOTH conditions are met:
  1. The file path contains dynamic segments like [id], [slug], [...path]
  2. The original code actually uses router.query or similar to access those parameters
- ONLY use useSearchParams() if the original code uses router.query for query parameters (?param=value)
- If the original code doesn't access any route parameters or search params, do NOT add these hooks
...
- Example: app/posts/page.tsx with no router.query usage → do NOT add any params hooks

Task:
- Migrate this file for the App Router.
- Create additional files ONLY if essential for this segment (layout.tsx, not-found.tsx, error.tsx) or if referenced.
- Prefer minimal changes preserving behavior.
...
def iter_source_files(root: str, include_globs: List[str], exclude_globs: List[str]) -> Iterable[str]:
...
# ---------------- Processing ----------------
def process_file(full_path: str, repo_cwd: str, project_root: str, write: bool) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if isinstance(parsed, dict) and "files" in parsed and isinstance(parsed["files"], list):
            ...
            for item in parsed["files"]:
                ...
                abs_path = os.path.join(repo_cwd, path) if not os.path.isabs(path) else path
                if not write:
...
def main():
    parser = argparse.ArgumentParser(description="Migrate Next.js files to App Router using Azure OpenAI, with optional scaffolding.")
    parser.add_argument("--root", default="services/main-frontend/src/app", help="App directory root")
    parser.add_argument("--include", nargs="*", default=[], help="Glob(s) to include relative to --root (e.g. '**/page.tsx')")
    parser.add_argument("--exclude", nargs="*", default=[], help="Glob(s) to exclude (e.g. '**/components/**')")
    parser.add_argument("--write", action="store_true", help="Write changes (otherwise dry-run)")
    parser.add_argument("--scaffold", action="store_true", help="Create root special files if missing (layout.tsx, not-found.tsx, error.tsx)")
    ...
    for path in iter_source_files(app_root, args.include, args.exclude):
        total += 1
        try:
            did_change, newly_created = process_file(path, os.getcwd(), app_root, args.write)
            if did_change:
                changed += 1
    ...
    logging.info("Done. Files scanned: %d; changed: %d; created: %d. Mode: %s",
                 total, changed, len(created), "WRITE" if args.write else "DRY-RUN")
