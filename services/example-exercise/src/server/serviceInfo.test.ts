...
describe("GET /api/service-info", () => {
  it("returns the service metadata with base-path-prefixed endpoint paths", async () => {
    ...
    // In tests PUBLIC_BASE_PATH is unset, so paths are unprefixed. The prefix is applied verbatim
    // from basePath() (validated end-to-end via the built server), so assert the suffixes.
