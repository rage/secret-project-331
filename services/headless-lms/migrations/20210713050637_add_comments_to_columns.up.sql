...
COMMENT ON COLUMN organizations.slug IS 'A unique abbreviation of the organization name that is visible in URLs e.g. "uh-cs".';
-- courses
...
COMMENT ON COLUMN courses.slug IS 'A unique abbreviation of the course name that is visible in URLs e.g. "introduction-to-birds".';
...
COMMENT ON COLUMN pages.content IS 'Page content in an abstract form. It is an array of JSON objects that are blocks. For example,a block could be a paragraph, or a image or an exercise.';
COMMENT ON COLUMN pages.url_path IS 'Relative url to the page within a course. It is autogenerated by default in the frontend for ease of use, but can be customized if desired. Example: "/part-1/3-hawaiian-crow".';
...
COMMENT ON COLUMN submissions.data_json IS 'The content of the submission. We do not know what is the structure of this field as it depends on the exercise service.';
...
COMMENT ON COLUMN exercise_tasks.assignment IS 'Exercise assignment (e.g. "In this exercise prove the statement P=NP.") in an abstract form. Uses exactly the same format as pages.content.';
...
COMMENT ON COLUMN gradings.submission_id IS 'The submission being graded.';
...
COMMENT ON COLUMN gradings.grading_priority IS 'If there is a queue with the gradings, gradings with lower value will be processed first.';
...
COMMENT ON COLUMN gradings.score_given IS 'How many points were given in this grading. This has been scaled to the max points of the exercise.';
-- column will be renamed in the next pr
COMMENT ON COLUMN gradings.unscaled_max_points IS 'Score maximum we got from the exercise service when doing the grading. This has not been adjusted to the max score specified in the exercise.';
-- column will be renamed in the next pr
COMMENT ON COLUMN gradings.unscaled_score_maximum IS 'How many points the exercise service wanted to give from this exercise. This has not been adjusted to the max score that was specified in the exercise.';
...
COMMENT ON COLUMN gradings.feedback_json IS 'Feedback from grading in a general form. We do not know the structure of this and it is up to the frontend to decide how it is rendered. Alternatively this can be omitted and feedback_text can be set.';
...
COMMENT ON COLUMN regrading_submissions.regrading_id IS 'The regrading operation to which the submission is attached to.';
...
COMMENT ON COLUMN users.upstream_id IS 'User''s id in upstream system (currently TMC). It is used to interact with the upstream authentication system. Internally the preferred identifier is user.id.';
COMMENT ON COLUMN users.email IS 'Email address that can be used to reach the user. Can change, do not use this as an identifier.';
-- roles
...
COMMENT ON COLUMN roles.course_id IS 'If not null, scopes the role to be valid only within a course.';
...
COMMENT ON COLUMN course_instances.starts_at IS 'Timestamp when the course instance is available to the students.';
COMMENT ON COLUMN course_instances.ends_at IS 'Timestamp when the course instance ceases to be available to the students.';
...
COMMENT ON COLUMN user_exercise_states.score_given IS 'If not null, it indicates how many points the student has gotten from this exercise.';
...
COMMENT ON COLUMN exercise_services.public_url IS 'A url to the service info endpoint that is accessible from the internet for enabling communication with the services outside the cluster. The base url of the service is derived from this and given to browser frontends so that they can access the iframes of this exercise service.';
COMMENT ON COLUMN exercise_services.internal_url IS 'A url to the service info endpoint that is accessible only from inside the cluster for enabling communication with the service without leaving the cluster. If not set, public_url will be used instead.';
...
COMMENT ON COLUMN email_templates.content IS 'Content of the email in an abstract form. Uses the same structure for the data as pages.content but the allowed blocks here is limited. The html and plain text versions of the email are generated from this.';
COMMENT ON COLUMN email_templates.name IS 'Human readable name of the email template that is used in listings in the teacher dashboard.';
