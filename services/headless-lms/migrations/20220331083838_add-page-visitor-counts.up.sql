...
CREATE TABLE page_visit_datum_daily_visit_hashing_keys (
  ...
  -- 1024 bytes is the default because it's the maximum postgres is willing to generate
  hashing_key BYTEA NOT NULL DEFAULT gen_random_bytes(1024),
...
COMMENT ON COLUMN page_visit_datum_daily_visit_hashing_keys.hashing_key IS 'Cyrptographically strong random data to be used in hashing. Postgres generates the values for this column on insert.';
...
COMMENT ON COLUMN page_visit_datum.page_id IS 'The page that was visited.';
