...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON course_student_average FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();
COMMENT ON TABLE course_student_average IS 'This table stores data regarding the average in a specific course.';
COMMENT ON COLUMN course_student_average.course_instance_id IS 'A unique, stable identifier for the record.';
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON suspected_cheaters_exercise_list FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON exercise_student_average FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();
