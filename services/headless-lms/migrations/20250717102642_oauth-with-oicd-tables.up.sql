...
COMMENT ON COLUMN oauth_clients.scopes IS
  'Default scopes the client may request if none provided at runtime (array)';
COMMENT ON COLUMN oauth_clients.origin IS
  'Only allowed origin for requests coming from this client';
...
CREATE TABLE oauth_auth_codes (
  ...
  CONSTRAINT scopes_token_chars CHECK (scopes_all_valid(scopes)),
  CONSTRAINT expires_at_reasonable CHECK (
...
COMMENT ON COLUMN oauth_auth_codes.scopes IS
  'Array of scopes granted by this code';
...
COMMENT ON COLUMN oauth_access_tokens.digest IS
  'Opaque access token digest (hashed at rest)';
...
COMMENT ON COLUMN oauth_access_tokens.scopes IS
  'Array of scopes granted to this token';
...
COMMENT ON COLUMN oauth_refresh_tokens.scopes IS
  'Array of scopes associated with this refresh token';
