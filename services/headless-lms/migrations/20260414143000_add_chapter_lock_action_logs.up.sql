CREATE TYPE chapter_lock_action_type AS ENUM (
  'teacher-lock',
  'teacher-unlock'
);
...
CREATE TABLE chapter_lock_action_logs (
  ...
  action chapter_lock_action_type NOT NULL,
  reason TEXT,
  source TEXT,
...
COMMENT ON COLUMN chapter_lock_action_logs.action IS 'Type of lock action (teacher-lock or teacher-unlock).';
COMMENT ON COLUMN chapter_lock_action_logs.reason IS 'Optional free-form reason for the lock action.';
COMMENT ON COLUMN chapter_lock_action_logs.source IS 'Optional source identifier for the feature/workflow that emitted the action.';
