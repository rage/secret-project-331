...
COMMENT ON COLUMN user_details.search_helper IS 'Helps us to search users with one trigram-indexed column. It contains user id, email, first-name last-name, and last-name first-name; the repeated first name is intentional so partial name searches work in either order.';
