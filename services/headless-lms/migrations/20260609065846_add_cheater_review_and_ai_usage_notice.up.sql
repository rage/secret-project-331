...
CREATE UNIQUE INDEX user_ai_usage_notice_ack_user_clg_uniqueness ON user_ai_usage_notice_acknowledgements (user_id, course_language_group_id)
WHERE deleted_at IS NULL;
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON user_ai_usage_notice_acknowledgements FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();
COMMENT ON TABLE user_ai_usage_notice_acknowledgements IS 'Records that a user has read and agreed to the AI-usage / academic-integrity notice for a course language group. One acknowledgement per user per course language group, so accepting the notice on one language version covers all language versions.';
...
COMMENT ON COLUMN user_ai_usage_notice_acknowledgements.course_language_group_id IS 'The course language group the acknowledgement applies to. Shared across all language versions of the course.';
