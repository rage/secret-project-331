...
mod test {
    ...
    /// A wrapped `BackendError` cause (here a `UtilError`) is rendered as its own node
    /// in the chain — not as an opaque `(external)` leaf. Exercises the cross-crate
    /// downcast resolver generated by `impl_clean_debug!`.
    #[test]
