...
use crate::library::pages::save_pipeline::{
    OldPageSaveContext, old_context_from_blocks, preprocess_cms_page_update,
};
...
pub async fn update_page(
    ...
    let old_context: OldPageSaveContext = old_context_from_blocks(&old_blocks);
    cms_page_update.validate_exercise_data()?;
    let content = preprocess_cms_page_update(&mut cms_page_update, &old_context)?;
...
pub async fn insert_page(
    ...
    let mut cms_page_update = CmsPageUpdate {
        content: replace_duplicate_client_ids(new_page.content.clone()),
    ...
    let content = preprocess_cms_page_update(
        &mut cms_page_update,
        &OldPageSaveContext {
            old_exercise_ids: vec![],
        },
    )?;
