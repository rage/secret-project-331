...
pub async fn fetch_exam_for_user(
    ...
    let enrollment = match exams::get_enrollment(&mut conn, *exam_id, user.id).await? {
        Some(enrollment) => {
            if exam.grade_manually {
                ...
                let mut grading_decision_and_exercise_list: Vec<(
                    ...
                    if let Some(hidden) = grading_decision.hidden {
                        if !hidden {
                            ...
                            for grading in teacher_grading_decisions.into_iter() {
                                ...
                                grading_decision_and_exercise_list
                                    .push((grading, exercise.clone()));
                            }
            ...
            if Utc::now() < ends_at
                && (Utc::now()
                    > enrollment.started_at + Duration::minutes(exam.time_minutes.into())
