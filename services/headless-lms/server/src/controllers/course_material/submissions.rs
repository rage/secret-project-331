...
async fn post_submission(
    ...
    for submission in task_submissions {
        ...
        let exercise_slide =
            exercise_slides::get_exercise_slide_by_exercise_task_id(&mut conn, exercise_task_id)
                .await?
                .ok_or_else(|| {
                    ControllerError::NotFound("Exercise definition not found.".to_string())
                })?;
        let exercise = exercises::get_by_id(&mut conn, exercise_slide.exercise_id).await?;
