...
/// Accepted submission archive extensions. tmc-langs sets no trustworthy content-type on
...
fn validate_submission_file_name(file_name: Option<&str>) -> Result<(), ControllerError> {
    let file_name = file_name.ok_or_else(|| {
        controller_err!(
            BadRequest,
            "The submission upload is missing a file name".to_string()
        )
    })?;
    if !filename_has_accepted_archive_extension(file_name) {
        return Err(controller_err!(
            BadRequest,
            format!(
                "The submission upload '{}' is not an accepted archive (expected one of: {})",
                file_name,
                ACCEPTED_ARCHIVE_EXTENSIONS.join(", ")
            )
        ));
    }
    Ok(())
...
async fn get_course_exercises(
    ...
    for open_exercise in open_chapter_exercises {
        ...
        let tasks: Vec<api::ExerciseTask> = slide
            ...
            // Model solutions are never revealed in this list view; the per-user
            // "reveal once solved" gate lives in `get_exercise`.
            .map(|mut et| {
...
async fn submit_exercise(
    ...
    validate_submission_file_name(temp_file.file_name.as_deref())?;
    let mime = submission_content_type(temp_file.content_type);
...
mod tests {
    ...
    #[test]
    fn accepted_archive_extensions_are_recognized() {
        assert!(filename_has_accepted_archive_extension("submission.zip"));
        assert!(filename_has_accepted_archive_extension(
            "submission.tar.zst"
        ));
        assert!(filename_has_accepted_archive_extension("SUBMISSION.ZIP"));
        assert!(!filename_has_accepted_archive_extension(
        ...
        assert!(!filename_has_accepted_archive_extension("submission.txt"));
        assert!(!filename_has_accepted_archive_extension("submission"));
        // The extension must be a suffix, not merely present in the name.
        assert!(!filename_has_accepted_archive_extension("zip.exe"));
    }

    /// A multipart `file` part without a filename gives us nothing to validate the archive by, so
    /// it must be rejected rather than stored under a guessed name.
    #[test]
    fn submission_without_a_file_name_is_rejected() {
        use actix_web::ResponseError;
        use actix_web::http::StatusCode;
        let err = validate_submission_file_name(None)
            .expect_err("an upload without a file name must be rejected");
        assert_eq!(err.status_code(), StatusCode::UNPROCESSABLE_ENTITY);
    }

    #[test]
    fn submission_with_a_non_archive_file_name_is_rejected() {
        use actix_web::ResponseError;
        use actix_web::http::StatusCode;
        let err = validate_submission_file_name(Some("submission.tar.gz"))
            .expect_err("a non-archive upload must be rejected");
        assert_eq!(err.status_code(), StatusCode::UNPROCESSABLE_ENTITY);
        // The message names the accepted extensions so the client can report something useful.
        assert!(err.to_string().contains(".tar.zst"), "{err}");
    }

    #[test]
    fn submission_with_an_accepted_archive_file_name_is_allowed() {
        assert!(validate_submission_file_name(Some("submission.zip")).is_ok());
        assert!(validate_submission_file_name(Some("submission.tar.zst")).is_ok());
    }

    /// tmc-langs uploads the archive without a content-type; that must default rather than fail,
    /// since the filename extension is the real gate.
    #[test]
    fn missing_content_type_falls_back_to_octet_stream() {
        assert_eq!(
            submission_content_type(None),
            mime::APPLICATION_OCTET_STREAM
        );
    }

    #[test]
    fn provided_content_type_is_preserved() {
        let zip: mime::Mime = "application/zip".parse().unwrap();
        assert_eq!(submission_content_type(Some(zip.clone())), zip);
    }

    /// The `submission` multipart part is deserialized into `api::ExerciseSlideSubmission` by
    /// `MultipartJson`, so a malformed part fails at the serde boundary before the handler body
    /// runs. Pin that contract: both ids are required and must be UUIDs.
    #[test]
    fn malformed_submission_part_fails_to_deserialize() {
        let valid = serde_json::json!({
            "exercise_slide_id": Uuid::new_v4(),
            "exercise_task_id": Uuid::new_v4(),
        });
        serde_json::from_value::<api::ExerciseSlideSubmission>(valid)
            .expect("a well-formed submission part deserializes");
