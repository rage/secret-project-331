...
// use models::suspected_cheaters::SuspectedCheaters;
// use models::suspected_cheaters::Threshold;
// use models::suspected_cheaters::SuspectedCheatersData;

// use crate::prelude::*;

/**
POST `/api/v0/main-frontend/suspected_cheater/threshold/:course_id` - Sets threshold for a course.

# Example

Request:
```http
POST /api/v0/main-frontend/suspected_cheater/threshold/65fd7f2c-09ae-52b2-806c-5649bdcb40e6 HTTP/1.1
...
//     user: AuthUser,
// ) -> ControllerResult<web::Json<Threshold>> {
//     let mut conn = pool.acquire().await?;
//     let new_threshold = payload.0;
//     let points = new_threshold.data.points;
//     let course_id = new_threshold.course_id;

//     let res =
//         models::suspected_cheaters::insert_thresholds(&mut conn, course_id, new_threshold).await?;

//     let token = authorize(&mut conn, Act::Edit, Some(user.id), Res::Threshold).await?;
...
//     cfg.route("/{course_instance_id}/threshold", web::post().to(insert_threshold))}
