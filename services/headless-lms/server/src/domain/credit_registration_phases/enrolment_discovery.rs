...
async fn reconcile(
    ...
    for person in people {
        ...
        } = claim_linking_mails(conn, &discovered).await?;
        outcome.mailed_count += claimed;
        outcome.suppressed_by_dedup_count += suppressed_by_dedup;
        outcome.suppressed_by_rate_cap_count += suppressed_by_rate_cap;
    }
