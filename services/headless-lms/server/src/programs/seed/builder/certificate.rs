...
impl CertificateBuilder {
    ...
    pub async fn seed(self, conn: &mut PgConnection) -> Result<Uuid> {
        ...
        if self.ensure_requirements {
            let reqs = certificate_configuration_to_requirements::get_all_requirements_for_certificate_configuration(
                conn, config_id,
            )
            .await
            .context("get_all_requirements_for_certificate_configuration")?;

            for course_module_id in reqs.course_module_ids {
                // fetch course_id + email for the completion insert
