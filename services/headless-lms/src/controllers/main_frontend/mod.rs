...
use self::{
    ...
    exercises::_add_exercises_routes, feedback::_add_feedback_routes, org::_add_org_routes,
...
pub fn add_main_frontend_routes(cfg: &mut ServiceConfig) {
    cfg.service(web::scope("/chapters").configure(_add_chapters_routes))
        .service(web::scope("/course-instances").configure(_add_course_instances_routes))
        .service(web::scope("/courses").configure(_add_courses_routes))
