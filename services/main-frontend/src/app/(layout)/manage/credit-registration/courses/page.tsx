...
/** Crimson only for a status that needs fixing now; a warning or unchecked module keeps the row's
 *  own ink, matching Badge's own choice not to trust yellow as foreground colour. */
const statusIconDangerCss = css`
...
const ModuleStatusMark: React.FC<{ module: CreditRegistrationCourseStats }> = ({ module }) => {
  ...
  if (StatusIcon === null) {
    return <span className={noteCss}>{label}</span>
  }
  return (
    <span className={statusMarkCss}>
      <StatusIcon
        size={STATUS_ICON_SIZE}
        className={cx(
          statusIconCss,
          courseModuleStatusTone(status) === TONE.DANGER && statusIconDangerCss,
        )}
      />
      {label}
    </span>
      ...
      <section className={sectionCardCss}>
        ...
        >
          {(stats) => {
            ...
            return (
              <>
                <StatTileList ariaLabel={t("credit-registration-heading-courses")} maxColumns={2}>
                  <StatTile
                  ...
                  <div className={controlCss}>
                    <Select
                      ...
                      options={[
                        {
                          value: SORT_NAME,
                          label: t("credit-registration-admin-sort-course-name"),
                        },
                        {
                          value: SORT_FAILURES,
