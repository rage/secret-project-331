...
const CourseEnrollmentsList: React.FC<CourseEnrollmentsListProps> = ({ userId }) => {
  ...
  return (
    <QueryResult query={courseEnrollmentsQuery}>
      {(data) => (
        <div>
          {data.course_enrollments.map((enrollment) => {
            const {
              course,
            ...
            const noCourseModuleCompletedTwice =
              course_module_completions.length === numDistinctModules
