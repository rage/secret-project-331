...
export const ValidityRegexTesterModalContent: React.FC<ModalContentProps> = ({ item }) => {
  ...
  const checkTestAnswer = (): boolean => {
    ...
    const cleanedInput = test.replace(/\0/g, "").replace(nonPrintingCharRegex, "")
  ...
  return (
    <>
      <ModalContent>
        <p>{t("regular-expression-tester")}</p>
      </ModalContent>
      <ModalContent>
        <TextField
        ...
        >
          {t("regular-expression-preview", { expression: variables.regex })}
        </TextField>
      </ModalContent>
      <ModalContent>
        <TextField
          ...
          variant="outlined"
          onChange={(event) =>
            dispatch(setValidityRegexTestAnswer(storeItem.id, event.target.value))
          }
        />
      </ModalContent>
      <ModalContent>
        <ModalTextContainer>
          {variables.regex ? (
                ...
                <>
                  {checkTestAnswer() ? (
                    <>
                      ...
                      <p>{t("given-text-matches-regular-expression")}</p>
                    </>
                  ) : (
                    <>
                      <IconContainer>
                      ...
                      <p>{t("given-text-does-not-match-regular-expression")}</p>
                    </>
                  )}
                </>
              ) : (
                <>
                  <p>{t("invalid-regular-expression")}</p>
                </>
              )}
            </>
          ) : (
            <>
              ...
              <p>{t("validity-regular-expression-not-provided")}</p>
            </>
          )}
        </ModalTextContainer>
      </ModalContent>
