...
// Base path this service is mounted under behind the ingress, e.g. "/tmc". Fixed at build time (the
// Dockerfile exports PUBLIC_BASE_PATH before `rsbuild build`) and auto-inlined into both bundles,
// so the client router and server routes agree.
...
export default defineConfig({
  plugins: [
    ...
    pluginReact(),
    // The answer-exercise view extracts tar.zst stubs in the browser (tar-stream + zstddec need the
    // events/buffer/stream node builtins). Next polyfilled these automatically; rsbuild needs this.
    pluginNodePolyfill(),
    pluginSvgr({
      svgrOptions: {
