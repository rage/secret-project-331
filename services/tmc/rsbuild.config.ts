...
// Base path this service is mounted under behind the nginx ingress, e.g. "/tmc". Fixed at build
...
export default defineConfig({
  plugins: [
    ...
    pluginReact(),
    // The answer-exercise view extracts tar.zst stubs in the browser (tar-stream + zstddec), which
    // rely on the events/buffer/stream node builtins. Next.js polyfilled these on the client
    // automatically; rsbuild needs this plugin (it only affects browser bundles).
    pluginNodePolyfill(),
    pluginSvgr({
      svgrOptions: {
