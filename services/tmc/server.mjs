...
// Trailing slashes trimmed so "/tmc" matches cleanly.
  ...
  try {
    ...
    if (isGet) {
      ...
      if (file) {
        ...
        // Hashed build assets and the versioned pyodide runtime never change in place.
        const cache =
          rel.includes("/assets/") || rel.startsWith("/pyodide/")
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`tmc listening on http://0.0.0.0:${PORT}${BASE_PATH || ""}`)
})
