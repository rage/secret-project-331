...
import {
  answerFilesToAnswerFileRefs,
  storedAnswerToAnswerExerciseFields,
  storedAnswerToCapturedAnswerFields,
...
const jsonAnswer = { data_json: { answer: "hello" } } as const
const fileAnswer = {
  data_json: { display_names: ["a", "b"] },
  data_files: [FIRST_FILE, SECOND_FILE],
}

describe("storedAnswerToViewSubmissionFields", () => {
  test("a json answer yields its data and no files field", () => {
    const fields = storedAnswerToViewSubmissionFields(jsonAnswer)
  ...
  test("a file answer yields the files and its metadata as the answer", () => {
    expect(storedAnswerToViewSubmissionFields(fileAnswer)).toEqual({
  ...
  test("keeps the host's file order rather than reordering", () => {
    const fields = storedAnswerToViewSubmissionFields({
      data_files: [SECOND_FILE, FIRST_FILE],
    })
  ...
  test("a file answer without metadata yields a null answer", () => {
    expect(storedAnswerToViewSubmissionFields({ data_files: [FIRST_FILE] })).toEqual({
      user_answer: null,
  ...
  test("a missing answer yields a null answer and no files field", () => {
    expect(storedAnswerToViewSubmissionFields(null)).toEqual({ user_answer: null })
    expect(storedAnswerToViewSubmissionFields(undefined)).toEqual({ user_answer: null })
  })

  test("a file stored before sizes were recorded reports no size instead of zero", () => {
    const fields = storedAnswerToViewSubmissionFields({
      data_files: [{ ...FIRST_FILE, size_bytes: null }],
    })
...
describe("storedAnswerToAnswerExerciseFields", () => {
  test("a json answer yields its data and no files field", () => {
    const fields = storedAnswerToAnswerExerciseFields(jsonAnswer)
    expect(fields).toEqual({ previous_submission: { answer: "hello" } })
    expect("previous_submission_files" in fields).toBe(false)
  })

  test("a file answer yields the files and its metadata as the answer", () => {
    expect(storedAnswerToAnswerExerciseFields(fileAnswer)).toEqual({
  ...
  test("a missing answer yields a null answer and no files field", () => {
    expect(storedAnswerToAnswerExerciseFields(null)).toEqual({ previous_submission: null })
  })
})

describe("storedAnswerToCapturedAnswerFields", () => {
  test("a json answer yields its data and no files field", () => {
    const fields = storedAnswerToCapturedAnswerFields(jsonAnswer)
  ...
  test("a file answer yields the host file ids in order", () => {
    expect(storedAnswerToCapturedAnswerFields(fileAnswer)).toEqual({
  ...
  test("a missing answer yields a null answer and no files field", () => {
    expect(storedAnswerToCapturedAnswerFields(undefined)).toEqual({ data: null })
  })
})

describe("answerFilesToAnswerFileRefs", () => {
  test("yields the files in the host's order", () => {
    expect(answerFilesToAnswerFileRefs(fileAnswer.data_files)).toEqual([FIRST_FILE, SECOND_FILE])
  })

  test("an answer with no files yields none", () => {
    expect(answerFilesToAnswerFileRefs([])).toBeUndefined()
    expect(answerFilesToAnswerFileRefs(null)).toBeUndefined()
    expect(answerFilesToAnswerFileRefs(undefined)).toBeUndefined()
  })
})
