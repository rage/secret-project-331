...
test.describe("Chatbot settings testing", () => {
  ...
  test("Creating a chatbot", async ({ page }) => {
    await accessibilityCheck(page, "/courses/id/other/chatbot", [])
    await page.getByRole("button", { name: "Create a new chatbot" }).click()
    await page.getByRole("textbox", { name: "Namefalse" }).click()
    await page.getByRole("textbox", { name: "Namefalse" }).fill("Chatbot 1")
  ...
  test("Editing a chatbot", async ({ page }) => {
    ...
    await accessibilityCheck(page, "/chatbots/id", [])
    await page.getByRole("textbox", { name: "Namefalse" }).click()
    await page.getByRole("textbox", { name: "Namefalse" }).fill("Chatbot 1")
    await page.getByRole("button", { name: "Save" }).click()
    ...
    await page.getByRole("textbox", { name: "Initial messagefalse" }).click()
    await page.getByRole("textbox", { name: "Initial messagefalse" }).fill("Hi! :)")
    await page.getByText("Enabled to students").click()
    await page.getByRole("heading", { name: "Advanced settings" }).click()
    await page.getByRole("spinbutton", { name: "Frequency penaltyfalse" }).click()
    await page.getByRole("spinbutton", { name: "Frequency penaltyfalse" }).fill("0.2")
  ...
  test("Accessibility check", async ({ page }) => {
    await accessibilityCheck(page, "IDKContext", [])
  })
})
