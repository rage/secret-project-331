...
test('Test course export', async ({ page }) => {
  await test.step("Upload an image to be backed up", async () => {
    ...
    await page.getByRole('row', { name: 'Page 4 /chapter-1/page-4 Edit page Dropdown menu' }).getByRole('button').first().click();
  ...
  await test.step("Export the course", async () => {
    ...
    await page.getByLabel('University of Helsinki, Department of Mathematics and Statistics').click();
    await page.getByLabel('Manage course \'Change path\'').click();
    await page.goto('http://project-331.local/cms/courses/c783777b-426e-4cfd-9a5f-4a36b2da503a/export');
    ...
    expect(allFiles).toEqual(expect.arrayContaining(expectedFiles))
