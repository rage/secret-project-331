...
const BACKFILL_COURSE_ID = "c5ed17ea-0003-4a5e-9e6e-c0de00000003"
    ...
    const rows = await pollUntil(
      async () => {
        const response = await page.request.get(
          `${CREDIT_REGISTRATION_ADMIN_API}/registrations?course_id=${BACKFILL_COURSE_ID}&limit=50`,
        )
        if (!response.ok()) {
          return null
        }
        const listed = (await response.json()) as {
          data: { email: string; state: string }[]
          total_count: number
        }
...
test("Late consent from the profile page unblocks the backfilled rows", async ({ page }) => {
  await loginAsSeededStudent(page, LATE_CONSENT_EMAIL)
